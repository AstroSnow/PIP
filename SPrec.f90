subroutine SPrec
  ...
  double precision theta_p,phi_p,tmp,v_L(8),v_R(8),wtr,ioneq,Te_0,tfac,n1(6),tstep,yloc
...
!  !set ionization fraction-----------------
  ...
  start(1)=-10.0d0 ;end(1)=10.0d0          !400.0d0/f_p
  start(2)=-10.0d0 ;end(2)=10.0d0
...
!  B0=sqrt(2.0*gm/beta)
  B0=1.0d0
  wtr=0.1d0 !some scaling factor for the width of the current sheet
  yloc=5.d0
!  B0=0.0d0
!/sqrt(2.0)
...
!0.001d0
  do k=1,kx;do j=1,jx;do i=1,ix
     ro_h(i,j,k)=f_n*v_l(1)
     ro_m(i,j,k)=f_p*v_l(1)
     p_h(i,j,k)=f_p_n*v_l(2)
     p_m(i,j,k)=f_p_p*v_l(2)
     vx_h(i,j,k)=0.d0!(v_l(3)+(v_r(3)-v_l(3))*(1.0d0+tanh(mask(i,j,k)/wtr))*0.5d0)
     vx_m(i,j,k)=0.d0!(v_l(3)+(v_r(3)-v_l(3))*(1.0d0+tanh(mask(i,j,k)/wtr))*0.5d0)
     vy_h(i,j,k)=0.d0!(v_l(4)+(v_r(4)-v_l(4))*(1.0d0+tanh(mask(i,j,k)/wtr))*0.5d0)
     vy_m(i,j,k)=0.d0!(v_l(4)+(v_r(4)-v_l(4))*(1.0d0+tanh(mask(i,j,k)/wtr))*0.5d0)
     vz_h(i,j,k)=0.d0!(v_l(5)+(v_r(5)-v_l(5))*(1.0d0+tanh(mask(i,j,k)/wtr))*0.5d0)
     vz_m(i,j,k)=0.d0!(v_l(5)+(v_r(5)-v_l(5))*(1.0d0+tanh(mask(i,j,k)/wtr))*0.5d0)
     b_x(i,j,k)=B0+B0*tanh((y(j)-yloc)/wtr)-B0*tanh((y(j)+yloc)/wtr)!-tanh((y(j)+yloc)/wtr))
     b_y(i,j,k)=0.d0
     b_z(i,j,k)=B0/cosh((y(j)-yloc)/wtr)-B0/cosh((y(j)+yloc)/wtr)!-0.5*cosh((y(j)+yloc)/wtr))
  enddo;enddo;enddo

print*,'Alfven speed range',maxval((b_x**2+b_y**2+b_z**2)/sqrt(ro_m+ro_h)),minval((b_x**2+b_y**2+b_z**2)/sqrt(ro_m+ro_h))
