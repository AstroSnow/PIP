subroutine asym_currentsheet
  ...
  double precision thick,b0,Bz0,inc_angle,b1,b2
...
!     print *,"CS"
     call set_coordinate_NUG(mpi_siz(1)*(ix-2*margin(1))+2*margin(1),&
          ix,x,dx,dxc,s_order,0.0d0,2.0d0,dx0,factor,1.0d3,mpi_pos(1),margin(1),1)
...
!  b_perp=B0*tanh(mask/thick)
  b_perp=-b2+(b1+B2)*(tanh(mask/thick)+1.0d0)/2.0d0
  b_tang=Bz0  
...
end subroutine asym_currentsheet
