subroutine NUwave
  ...
  use globalvar,only:ix,jx,kx,U_h,U_m,flag_bnd,col,beta,flag_b_stg,dtout,&
       ...
       flag_resi,eta_0,margin,dxc,s_order,mpi_siz,mpi_pos
  ...
  double precision f_n,f_p,f_p_n,f_p_p,start(3),end(3),omega_r,omega_i,L_x
  ...
  if(debug_direction.eq.1) then
     call set_coordinate_NUG(mpi_siz(1)*(ix-2*margin(1))+2*margin(1),&
          ix,x,dx,dxc,s_order,0.0d0,0.5d0,5.0d-3,2.0d0,1.0d-1,mpi_pos(1),margin(1),1)
     call set_fold_grid(ix,margin(1),x,dx,dxc,s_order)
!     L_x=0.5d0*(x(ix-margin(1))+x(ix-margin(1)+1)-(x(margin(1)+1)+x(margin(1))))
!     print *,dxc,dx,x
     L_x=1.0d0
     mask=spread(spread(2.0d0*pi*x/L_x,2,jx),3,kx)
     phi_p=0.0d0
     theta_p=pi/2.0d0
